...
**IMPORTANT:** When an idea exploration completes:
1. The `notify-research-complete.sh` script will send the file to Saved Messages
2. A wake event will be sent to ClaudeBot
3. **YOU MUST manually notify the user in the original chat/topic** where they requested the idea exploration
4. Message format: "✅ 研究完成：[idea title]\n📁 完整報告已傳送到 Saved Messages"

**Why:** Telegram Saved Messages don't show notifications, so users won't know the research is complete unless you tell them in the active chat.
